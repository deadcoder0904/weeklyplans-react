import React from "react";

const PageOne = () => (
  <div className="absolute top-0">
    <header className="bg-gold sans-serif">
      <div className="mw9 center pa2 pt3-ns ph7-l">
        <img src="assets/logo.png" alt="Logo" />
        <time className="f6 mb2 dib ttu tracked">
          <small>Created on 16th June, 2017</small>
        </time>
        <h3 className="headline measure-narrow lh-title mv0">
          <span className="bg-black-90 lh-copy white pa1 tracked-tight ph2">
            Write Weekly Plans in a whole new way.
          </span>
        </h3>
        <h4 className="f3 f4-m fw1 georgia i lh-title">
          Probably the easiest app to make Checklist for the upcoming Weeks.
        </h4>
        <h5 className="ma1 f6 ttu tracked black-80">By Akshay Kadam</h5>
      </div>
    </header>
    <div className="arrow animated bounce">
      <img
        width="40"
        height="40"
        alt=""
        src="data:image/svg+xml;base64,"
      />
    </div>
  </div>
);

export default PageOne;
